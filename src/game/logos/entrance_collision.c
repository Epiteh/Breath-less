...
static void logos_entrance_tp(rpg_t *rpg, sfColor color)
{
    if (is_same_color(color, (sfColor) {255, 10, 10, 255}) &&
        sfKeyboard_isKeyPressed(sfKeyY)) {
        init_tp_player(rpg, 1866, 1942);
        rpg->sprite.position.x = 1866;
        rpg->sprite.position.y = 1942;
        rpg->hud.player_pos = (sfVector2f) {2175 - 1659, 1757};
        scene.scene = "game";
    }
    if (is_same_color(color, (sfColor) {255, 11, 11, 255}) &&
        sfKeyboard_isKeyPressed(sfKeyY)) {
        init_tp_player(rpg, 465, 758);
        rpg->sprite.position.x = 465;
        rpg->sprite.position.y = 758;
